...
Planted so a correct run of the skill has to catch 4 problems, including one
...
| Description cross-check | Flagged as false | `pr_body.md` shows a guard clause `if (taxRate === 0) return null;` that does not appear anywhere in `pr.diff`. It's a fabricated example |
...
"handles the 0% case" claims at face value. It has to trace the actual diff.
